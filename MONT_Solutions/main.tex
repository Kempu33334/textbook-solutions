...
\end{problem}
We may write that \[\binom{p-1}{k} \equiv \dfrac{(p-1)!}{k!(p-1-k)!} \equiv \dfrac{-1}{k!(p-1-k)!} \equiv \dfrac{-1}{(-1)^k(p-1)!} \equiv (-1)^k \pmod{p}.\]
...
\end{problem}
Notice that for any prime $p$, selecting $n = p-2$ gives that \[2^{p-2}+3^{p-2}+6^{p-2}-1 \equiv \dfrac{1}{2}+\dfrac{1}{3}+\dfrac{1}{6}-1 \equiv 0 \pmod{p}\] so the only answer is $\boxed{1}$.
...
\end{enumerate}
\end{problem}
We start with the first problem. We notice that the power of $2$ doesn't matter, so let there be some positive integer $n = {p_1}^{e_1}{p_2}^{e_2}\cdots {p_m}^{e_m}{q_1}^{f_1}{q_2}^{f_2}\cdots {q_n}^{f_n}$ where all the $p$ are congruent to $1 \pmod{4}$ and all the $q$ are congruent to $3 \pmod{4}$. Now the $p$ don't matter either, since they add one to each count. 
\end{document}
