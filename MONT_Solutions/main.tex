...
If $n = 2$, then suppose that there exist infinitely many numbers $a_1$, $a_2 \in \mathbb{Q}$ such that $a_1+a_2$ and $a_1^{-1}+a_2^{-1}$ are integers. Then, $\tfrac{a_1}{a_2}+\tfrac{a_2}{a_1}$ must be an integer, but this is only true when $a_1 = a_2$, which is not allowed.
