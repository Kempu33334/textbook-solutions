...
Suppose that $n = p_1^{e_1}p_2^{e_2}\cdots p_k^{e_k}$ is in $G$ such that the $p$ are ordered increasingly. Furthermore, notice that $k = 1$ doesn't work, so we know that $k \ge 2$. Now, if $n \in G$, then clearly the smallest element of $A$ must be $p_1$, and the second smallest must be either $p_1^2$ or $p_2$. If it is $p_1^2$, then the progression will never hit any non-multiple of $p_1$, so it must be $p_2$. Now, 
