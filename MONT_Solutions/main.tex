...
We claim the only answer is all $k$. Let $n = p_1^{e_1}p_2^{e_2}\cdots p_{\ell}^{e_{\ell}}$. Then, the equation is equivalent to \[\dfrac{(2e_1+1)(2e_2+1)\cdots(2e_{\ell}+1)}{(e_1+1)(e_2+1)\cdots(e_{\ell}+1)} = k.\] Now, since the numerator is odd, we know that $2 \mid e_i$ over all $i$, so take the map $e_i \mapsto 2e_i$, transforming the equation into \[\dfrac{(4e_1+1)(4e_2+1)\cdots(4e_{\ell}+1)}{(2e_1+1)(2e_2+1)\cdots(2e_{\ell}+1)} = k.\] Clearly, this cannot be even, so we set to constructing odd $k$.
